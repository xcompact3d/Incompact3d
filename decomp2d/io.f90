...
contains
  ...
  if (ierror /= 0) then
     ...
     call decomp_2d_abort(__FILE__, __LINE__, ierror, "adios2_init")
  endif
  ...
  subroutine decomp_2d_io_finalise()
    ...
    if (ierror /= 0) call decomp_2d_abort(__FILE__, __LINE__, ierror, "adios2_finalize")
#endif
...
#include "io_write_every.inc"
  ...
  subroutine coarse_extents(ipencil, icoarse, sizes, subsizes, starts, opt_decomp)
    ...
    if ((ipencil < 1).or.(ipencil > 3)) then
       call decomp_2d_abort(__FILE__, __LINE__, ipencil, &
               "Error invalid value of ipencil")
    endif
  ...
  subroutine mpiio_write_real_coarse(ipencil,var,dirname,varname,icoarse,io_name,opt_decomp,reduce_prec,opt_deferred_writes)
    ...
    if (.not.var_handle % valid) then
       ...
       call decomp_2d_abort(__FILE__, __LINE__, ierror, "adios2_inquire_variable")
    endif
    ...
    if (engine_registry(idx)%valid) then
       ...
       call decomp_2d_abort(__FILE__, __LINE__, 0, "ERROR: decomp2d thinks engine is live, but adios2 engine object is not valid")
    end if
#endif
    ...
    if (io_handle%valid) then
       ...
       if (.not.var_handle % valid) then
          ...
          if (type == kind(0._real64)) then
             ...
             call decomp_2d_abort(__FILE__, __LINE__, 0, "adios2, wrong datatype")
          endif
          ...
          if (ierror /= 0) call decomp_2d_abort(__FILE__, __LINE__, ierror, "ERROR registering variable")
       endif
    else
       call decomp_2d_abort(__FILE__, __LINE__, 0, "Trying to register variable with invalid IO!")
    end if
#endif
  ...
  subroutine mpiio_write_real_probe(ipencil,var,filename,nlength)
    ...
    call MPI_FILE_OPEN(MPI_COMM_WORLD, filename, &
         ...
         fh, ierror)
    if (ierror /= 0) call decomp_2d_abort(__FILE__, __LINE__, ierror, "MPI_FILE_OPEN "//trim(filename))
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  subroutine write_subdomain(ipencil,var,is,ie,js,je,ks,ke,filename)
    ...
    if (color==1) then ! only ranks in this group do IO collectively
       ...
       if (ierror /= 0) call decomp_2d_abort(__FILE__, __LINE__, ierror, "MPI_FILE_CLOSE")
       call MPI_TYPE_FREE(newtype,ierror)
       if (ierror /= 0) call decomp_2d_abort(__FILE__, __LINE__, ierror, "MPI_FILE_FREE")
  ...
  subroutine decomp_2d_init_io(io_name)
    ...
    if (adios%valid) then
       ...
       if (ierror /= 0) call decomp_2d_abort(__FILE__, __LINE__, ierror, "adios2_declare_io")
    else
       call decomp_2d_abort(__FILE__, __LINE__, 0, "Couldn't declare IO - adios object not valid")
    end if    
#endif
    ...
    if (idx < 1) then
       ...
       if (nreg_io < MAX_IOH) then
          ...
          if (io%valid) then
             ...
             if (ierror /= 0) call decomp_2d_abort(__FILE__, __LINE__, ierror, "adios2_open, error opening engine")
          else
             call decomp_2d_abort(__FILE__, __LINE__, 0, "Couldn't find IO handle")
          end if
#endif
       end if
    end if
  ...
  subroutine decomp_2d_start_io(io_name, io_dir)
    ...
    associate(engine => engine_registry(idx))
      if (engine%valid) then
         ...
         call decomp_2d_abort(__FILE__, __LINE__, 0, "Trying to begin step with invalid engine")
      end if
    end associate
#endif
  ...
  subroutine decomp_2d_end_io(io_name, io_dir)
    ...
    associate(engine => engine_registry(idx))
      if (engine%valid) then
         ...
         call decomp_2d_abort(__FILE__, __LINE__, 0, "Trying to end step with invalid engine")
      end if
    end associate
#endif
