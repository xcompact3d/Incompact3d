...
module forces
 ...
 contains
   ...
   subroutine force(ux1,uy1,uz1,ep1)
     ...
     real(mytype) :: convxt,convyt,pressxt,pressyt,stressxt,stressyt
     ...
     if (i2dsim==1) then
       ...
       do iv=1, nvol
          ...
          call MPI_ALLREDUCE(convx, convxt, 1, real_type, MPI_SUM, MPI_COMM_WORLD, code)
          call MPI_ALLREDUCE(convy, convyt, 1, real_type, MPI_SUM, MPI_COMM_WORLD, code)
          call MPI_ALLREDUCE(pressx, pressxt, 1, real_type, MPI_SUM, MPI_COMM_WORLD, code)
          call MPI_ALLREDUCE(pressy, pressyt, 1, real_type, MPI_SUM, MPI_COMM_WORLD, code)
          call MPI_ALLREDUCE(stressx, stressxt, 1, real_type, MPI_SUM, MPI_COMM_WORLD, code)
          call MPI_ALLREDUCE(stressy, stressyt, 1, real_type, MPI_SUM, MPI_COMM_WORLD, code)
    
          pressxt = pressxt/dt
          pressyt = pressyt/dt 
          
          xDrag_mean = -xmom + convxt - pressxt -stressxt
          yLift_mean = -ymom + convyt - pressyt -stressyt
