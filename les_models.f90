...
subroutine smag(ux1,uy1,uz1,gxx1,gyx1,gzx1,gxy1,gyy1,gzy1,gxz1,gyz1,gzz1,&
  ...
  real(mytype),dimension(xsize(1),xsize(2),xsize(3)) :: ux1,uy1,uz1,di1

  real(mytype),dimension(xsize(1),xsize(2),xsize(3)) :: srt_smag,nut1
  real(mytype),dimension(ysize(1),ysize(2),ysize(3)) :: srt_smag2,nut2

  real(mytype),dimension(ysize(1),ysize(2),ysize(3)) :: ux2,uy2,uz2,di2,ta2
  real(mytype),dimension(zsize(1),zsize(2),zsize(3)) :: ux3,uy3,uz3,di3,ta3
  ...
  real(mytype),dimension(ysize(1),ysize(2),ysize(3)) :: syy2,szz2,sxy2,syz2
  ...
  if (ncly .eq. 2) then
    call derys (gxy2,ux2,di2,sy,ffypS,fsypS,fwypS,ppy,ysize(1),ysize(2),ysize(3),1)
    call derys (gyy2,uy2,di2,sy,ffyS,fsyS,fwyS,ppy,ysize(1),ysize(2),ysize(3),0)
    call derys (gzy2,uz2,di2,sy,ffypS,fsypS,fwypS,ppy,ysize(1),ysize(2),ysize(3),1)
  else
...
subroutine wale(gxx1,gyx1,gzx1,gxy1,gyy1,gzy1,gxz1,gyz1,gzz1,srt_smag,nut1)
  ...
  real(mytype),dimension(xsize(1),xsize(2),xsize(3)) :: gxx1,gyx1,gzx1,gxy1,gyy1,gzy1,gxz1,gyz1,gzz1,srt_smag,nut1
  real(mytype),dimension(xsize(1),xsize(2),xsize(3)) :: sxxd1,syyd1,szzd1,sxyd1,sxzd1,syzd1,srt_wale
  ...
  srt_wale=zero; srt_wale2=zero

  srt_wale = sxxd1*sxxd1 + syyd1*syyd1 + szzd1*szzd1 + two*(sxyd1*sxyd1 + sxzd1*sxzd1 + syzd1*syzd1)
  ...
  do k=1,ysize(3)
    do j=1,ysize(2)
      do i=1,ysize(1)
        nut2(i,j,k) = ((walecst*FSGS*del(j))**two)*((srt_wale2(i,j,k)**(three/two))/(srt_smag2(i,j,k)**(five/two)+srt_wale2(i,j,k)**(five/four)))
      enddo
    enddo
  enddo
  call transpose_y_to_x(nut2,nut1)
...
subroutine dynsmag(ux1,uy1,uz1,ep1,sxx1,syy1,szz1,sxy1,sxz1,syz1,&
  ...
  if (itime .gt. 1) then
   ...
   if (nclz.eq.0) then
    ...
    if (ncly .eq. 2) then
      call derys (gxy2f,ux2f,di2,sy,ffypS,fsypS,fwypS,ppy,ysize(1),ysize(2),ysize(3),1)
      call derys (gyy2f,uy2f,di2,sy,ffyS ,fsyS ,fwyS ,ppy,ysize(1),ysize(2),ysize(3),0)
      call derys (gzy2f,uz2f,di2,sy,ffypS,fsypS,fwypS,ppy,ysize(1),ysize(2),ysize(3),1)
    else
   ...
   if (nclz.eq.0) then
    ...
    do ijk=1,nvect1 !ERIC LIMITEUR SI BESOIN
      if (smagC1(ijk,1,1).gt. 0.4) smagC1(ijk,1,1) = zero
      if (smagC1(ijk,1,1).lt. 0.0) smagC1(ijk,1,1) = zero
    enddo

    if (nrank==0) print *, "smagC     min max= ",minval(smagC1),maxval(smagC1)
    ...
    do ijk=1,nvect1 !ERIC LIMITEUR SI BESOIN
      if (dsmagcst1(ijk,1,1).gt. 0.4) dsmagcst1(ijk,1,1) = zero
      if (dsmagcst1(ijk,1,1).lt. 0.0) dsmagcst1(ijk,1,1) = zero
    enddo

  else !itime=0

    dsmagcst1 = smagcst**two

  endif

#ifdef IBM
    dsmagcst1 = dsmagcst1*(one-ep1)
#endif
  ...
  if (nrank==0) print *, "dsmagcst     min max= ",minval(dsmagcst1),maxval(dsmagcst1)
  if (nrank==0) print *, "dynsmag nut1 min max= ",minval(nut1),maxval(nut1)
...
!************************************************************
subroutine lesdiff(ux1,uy1,uz1,sxx1,syy1,szz1,sxy1,sxz1,syz1,nut1,&
  ...
  if (ncly .eq. 2) then !!!!nut is odd
    call derys (ta2,nut2,di2,sy,ffyS,fsyS,fwyS,ppy,ysize(1),ysize(2),ysize(3),0)
  else
...
#ifndef STRETCHING
  if (ncly .eq. 2) then
    call deryys (te2,uy2,di2,sy,sfyS,ssyS,swyS,ysize(1),ysize(2),ysize(3),0)
  else
...
#ifndef STRETCHING
  if (ncly .eq. 2) then
    call deryys (tf2,uz2,di2,sy,sfypS,ssypS,swypS,ysize(1),ysize(2),ysize(3),1)
  else
